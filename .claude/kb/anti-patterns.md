...
| C08 | `MERGE`/CDC flow without explicit delete handling, fed by a CDC source with `REPLICA IDENTITY DEFAULT` | **Fixed 2026-06-20** (`DESIGN_DELETE_HANDLING.md`) — was confirmed via live test, not just inferred: with `REPLICA IDENTITY DEFAULT`, Postgres only logged PK columns in a DELETE's before-image, so the rewritten Kafka record had every non-key field `NULL`, and `create_auto_cdc_flow()` (no `apply_as_deletes`) applied it as `UPDATE SET *` — NULLing the Silver row instead of removing it. Fix: `sql/init.sql` now sets `REPLICA IDENTITY FULL` on all 20 tables (re-verified live post-fix: a delete-rewrite record now carries every real field value, matching its preceding insert/update exactly) + `register_silver()` passes `apply_as_deletes=expr("__op = 'd'")` + `register_silver_users()`'s dedup now excludes a `cpf` once its latest event is a delete. **Still unverified:** whether `apply_as_deletes` needs `full_refresh=true` to reprocess already-ingested history — no Databricks workspace available in this session to test. See `CLAUDE.md` and `.claude/sdd/features/DESIGN_DELETE_HANDLING.md`/`BUILD_REPORT_DELETE_HANDLING.md`. |
