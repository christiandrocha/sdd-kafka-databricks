...
| H11 | Databricks `MERGE`/CDC flow without explicit delete handling | `create_auto_cdc_flow()` calls in `register_silver()` don't pass `apply_as_deletes` — a Postgres hard delete (`__op='d'`) is not specially routed today; it lands as a normal upsert of whatever payload Debezium's `delete.handling.mode=rewrite` carries. Verify this is the intended behavior (soft-delete-as-update) before treating it as a bug — flag for `/design` if it isn't. |
